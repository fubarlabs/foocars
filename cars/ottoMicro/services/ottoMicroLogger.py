...
DEFAULT_AUTONOMOUS_THROTTLE = 1559
...
g_Mode_Collect_Data = False
g_Mode_Autonomous = False
g_User_Wants_To_Stop_Autonomous = False
...
g_laptop_data_path = 'jim@jim-XPS-13-9360.local:/home/jim/Dropbox/foocars/cars/ottoMicro/'
...
g_laptop_training_weights_file = 'jim@jim-XPS-13-9360.local:/home/jim/Dropbox/foocars/cars/ottoMicro/data/weights.h5'
g_pi_training_steerstats_file = '/home/pi/foocars/cars/ottoMicro/data/steerstats.npz'
g_pi_training_weights_file = '/home/pi/foocars/cars/ottoMicro/data/weights.h5'
...
def callback_switch_autonomous( channel ):
    ...
    global g_getter
    global g_ip_thread
    global g_User_Wants_To_Stop_Autonomous

    if( GPIO.input( SWITCH_autonomous ) == SWITCH_UP ):
        if( g_Mode_Autonomous == False ):
            try:
        ...
        if( g_Mode_Autonomous == True ):
            logging.debug( '* autonomous switch is now down' )
            g_User_Wants_To_Stop_Autonomous = True
            turn_OFF_LED( LED_autonomous )
...
class DataCollector(object):
    ...
    def __init__(self):
        ...
        try:        #   reorganize the data collection folder
            ...
            path = '/home/pi/foocars/cars/ottoMicro/data/collected_data'
...
def callback_switch_collect_data( channel ):
    global g_Recorded_Data_Not_Saved
    global g_Wants_To_See_Video
    global g_Mode_Collect_Data
    global g_camera
    global g_collector

    if( GPIO.input( SWITCH_collect_data ) == SWITCH_UP ):
        if( g_Mode_Collect_Data == False ):
            try:
                turn_ON_LED( LED_collect_data )
    ...
    if(( LED_count % 500000 ) == 0 ):       # every 500000 times through flip the led state to show we're alive
        ...
        if( g_Mode_Autonomous == False ):
            if( GPIO.input( SWITCH_autonomous ) == SWITCH_UP ):
                ...
                dontWaitForCommand = True     #  don't wait
                theCommandList = getSerialCommandIfAvailable( dontWaitForCommand )
                print( theCommandList )
                if( theCommandList[ 0 ] == commandEnum.STOP_AUTONOMOUS ):
                    logging.debug( 'rc wants to stop autonomous' )
                    stop_autonomous()
